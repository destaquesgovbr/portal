...
test.describe('Clipping — AI Agent Creation', () => {
  // Longer timeout: agent takes ~30-60s
  ...
  test('full flow: agent → accept → schedule → channels → confirm', async ({
    ...
    // Cleanup: find and store the created clipping ID
    const response = await page.request.get('/api/clipping')
    const clippings = await response.json()
    const agentClipping = clippings.find(
      (c: { name: string }) =>
        c.name.toLowerCase().includes('tecnologia') ||
        c.name.toLowerCase().includes('inovação'),
    )
    if (agentClipping) {
      createdClippingId = agentClipping.id
    }
  })
})
